...
create_vmess_URL_config() {
	...
	if [[ $v2ray_transport == [45] ]]; then
		...
		cat >/etc/v2ray/vmess_qr.json <<-EOF
			vless://${v2ray_id}@${domain}:443?encryption=none&security=tls&type=ws&host=${domain}&path=${_path}#${domain}-mluoc
		EOF
	else
		[[ -z $ip ]] && get_ip
		cat >/etc/v2ray/vmess_qr.json <<-EOF
...
update_v2ray.sh() {
	if [[ $_test ]]; then
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/mlch911/v2ray/test/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	else
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/mlch911/v2ray/master/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
